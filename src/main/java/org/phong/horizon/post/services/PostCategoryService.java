...
public class PostCategoryService {
    ...
    public PostCategory getPostCategoryById(UUID postCategoryId) {
    ...
    public PostCategory createPostCategory(CreatePostCategoryRequest request) {
        boolean isExist = postCategoryRepository.existsByName(request.categoryName());

        if (isExist) {
            throw new PostCategoryExistsException(PostErrorEnums.POST_CATEGORY_EXISTS.getMessage());
        }

        return postCategoryRepository.save(PostCategory.builder().name(request.categoryName()).build());
    }

    @PreAuthorize("hasRole('ROLE_ADMIN')")
    public PostCategorySummary updatePostCategory(UUID postCategoryId, UpdatePostCategoryRequest request) {
        PostCategory postCategory = getPostCategoryById(postCategoryId);
        ...
        mapper.partialUpdate(request, postCategory);
